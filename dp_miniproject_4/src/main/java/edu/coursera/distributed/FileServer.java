...
public final class FileServer {
	 ...
	 *            optional. You are free to use this information to change how
	 *            you create your threads, or ignore it.
	 * @throws IOException If an I/O error is detected on the server. This
	 *             should be a fatal error, your file server implementation is
	 *             not expected to ever throw IOExceptions during normal
	 *             operation.
	 */
	public void run(final ServerSocket socket, final PCDPFilesystem fs, final int ncores) throws IOException {
		...
		while (true) {
			 ...
			 *
			 * a) Using Socket.getInputStream(), parse the received HTTP packet.
			 * In particular, we are interested in confirming this message is a
			 * GET and parsing out the path to the file we are GETing. Recall
			 * that for GET HTTP packets, the first line of the received packet
			 * will look something like:
			 *
			 * GET /path/to/file HTTP/1.1 b) Using the parsed path to the target
			 * file, construct an HTTP reply and write it to
			 * Socket.getOutputStream(). If the file exists, the HTTP reply
